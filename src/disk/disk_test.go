...
var g = make([]byte, 1024)

func fill_block(size uint, input *os.File) ([]byte, bool) {
	n, _ := input.Read(g)
	return g, uint(n) == size
...
func Test_encoding(t *testing.T) {
	...
	//encoding is done
	i := LVD.NewErasureCoder([]byte{0, 1, 2}, []byte{0, 1, 2, 3, 4})
	out := i.Code(in)
	c2 := LVD.NewErasureCoder([]byte{0, 3, 4}, []byte{1, 2})
...
func Benchmark_encoding(b *testing.B) {

	in := make([][]byte, 3)
	i := LVD.NewErasureCoder([]byte{0, 1, 2}, []byte{0, 1, 2, 3, 4})
	eof := false
	for ddq := 0; ddq < b.N; ddq++ {
		fi, err := os.Open("test.dat")
		if err != nil {
			// panic()
		}
		for !eof {
